# Hi there, I'm Adibah 👋

🎓 **Master of Data Science @ Universiti Malaya**  
💻 **Data Analytics • Data Engineering • Machine Learning**  
📍 Kuala Lumpur, Malaysia

---

## 👩‍💻 About Me

I'm a data professional with experience across **data analytics, data engineering, and machine learning**, working with large-scale datasets to transform raw data into meaningful insights.

🔭 Currently exploring **Data Engineering, AI & Machine Learning**  
🎓 Master of Data Science from **Universiti Malaya**  
☁️ **CompTIA Cloud+ Certified**  
⚡ Experienced with **Databricks, Apache Spark, SQL & Python**  
📊 Interested in building scalable data solutions and data-driven applications

---

## 💼 Experience

### 📊 MIS Analyst — Capillary Technologies
**2025 – 2026 | Kuala Lumpur, Malaysia**

- Developed and maintained data analysis and reporting workflows using **Databricks, SQL and Apache Spark**
- Worked with large-scale retail datasets across multiple APAC markets
- Performed data validation, reconciliation and ad-hoc analytical studies
- Built recurring KPI reporting and customer analytics
- Supported data migration validation and cross-system reconciliation

### 💻 Data / Analytics — K Navitas
**Prior Experience | Malaysia**

- Worked on data projects within the banking industry
- Experience supporting **Alliance Bank, Bank Islam and MBSB Bank**
- Worked with **SQL Server, SSIS, Excel VBA, Python and SAS**

---

## 🛠️ Tech Stack

### 👩‍💻 Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=flat&logo=postgresql&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=flat&logo=r&logoColor=white)

### 🔥 Big Data & Data Engineering

![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat&logo=databricks&logoColor=white)
![Apache Spark](https://img.shields.io/badge/Apache%20Spark-E25A1C?style=flat&logo=apachespark&logoColor=white)
![Apache Hadoop](https://img.shields.io/badge/Apache%20Hadoop-66CCFF?style=flat&logo=apachehadoop&logoColor=black)
![SSIS](https://img.shields.io/badge/SSIS-CC2927?style=flat&logo=microsoftsqlserver&logoColor=white)

### 🗄️ Databases

![Microsoft SQL Server](https://img.shields.io/badge/Microsoft%20SQL%20Server-CC2927?style=flat&logo=microsoftsqlserver&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)

### ☁️ Cloud & Infrastructure

![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonwebservices&logoColor=white)
![EC2](https://img.shields.io/badge/Amazon%20EC2-FF9900?style=flat&logo=amazonec2&logoColor=white)

### 📊 Analytics & Visualization

![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white)
![Microsoft Excel](https://img.shields.io/badge/Microsoft%20Excel-217346?style=flat&logo=microsoftexcel&logoColor=white)
![SAS](https://img.shields.io/badge/SAS-1E90FF?style=flat&logoColor=white)

### 🤖 Data Science & Machine Learning

![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Scikit Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat&logo=scikitlearn&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)

### 🧰 Tools

![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat&logo=visualstudiocode&logoColor=white)

---

## 🎓 Education

### 🎓 Master of Data Science
**Universiti Malaya**

Research:
> **Soft Set–Based Approach for Modelling Uncertainty in R&D Investment Prioritization**

Focus areas:
- Machine Learning
- Big Data Analytics
- Data Mining
- Cloud Computing
- Statistical Analysis

---

## 🏆 Certifications

### ☁️ CompTIA Cloud+
**CompTIA Cloud+ Certified**

---

## 🚀 Featured Projects

### ✈️ Airline Demand & Loading Prediction
Machine learning project focused on predicting demand for airline loading decisions while considering temporal validation and data leakage.

`Python` `Machine Learning` `Feature Engineering` `Time-Series Validation`

### 🔬 R&D Investment Prioritization
Research project applying **Soft Set Theory and CRITIC weighting** to model uncertainty in R&D investment prioritization.

`Python` `Decision Analytics` `Soft Set Theory` `CRITIC`

### ⚡ Apache Spark vs Hadoop
Performance comparison of distributed big-data processing using **Apache Spark and Hadoop**.

`Apache Spark` `Hadoop` `Big Data`

### 🩺 Diabetes Prediction
Machine learning classification project for diabetes prediction.

`Python` `Scikit-learn` `Classification` `Data Analysis`

---

## 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=deebtarmizi104&show_icons=true&hide_border=true" height="165"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=deebtarmizi104&layout=compact&hide_border=true" height="165"/>
</p>

---

## 🌐 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Adibah-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/wan-nurul-adibah/)

[![Email](https://img.shields.io/badge/Email-Contact%20Me-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:wnadibahwt@gmail.com)

[![GitHub](https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/deebtarmizi104)

---

<p align="center">
  ✨ Turning data into insights, one query at a time.
</p>
